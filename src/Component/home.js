...
const heading = {heading: JSON.stringify(["Rank","Name","Platform","Year","Genre","Publisher","Global_Sales",""])}
...
const getSearchList = (text, data) => {
    ...
    return data.filter(game => Object.keys(game)[0].toLocaleUpperCase().match(regex))
}
...
export default class Home extends Component {
  ...
  handleChange = e => {
    ...
    console.log(getSearchList(search, this.state.data))
    this.setState(
      {
        searchText: search
      },
      () => {
        this.setState({
          searchList: [heading, ...getSearchList(search.toLocaleUpperCase(), this.state.data)]
        });
      }
    );
  };
  ...
  render() {
    const [first = {}, ...rest] = this.state.searchList.length > 1 ? this.state.searchList : this.state.data;
    console.log(first)
    let heading = [];
